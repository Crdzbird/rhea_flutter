...
class CircularPercentIndicator extends StatefulWidget {
  CircularPercentIndicator({
    ...
    assert(startAngle >= 0.0, 'Start angle must be >= 0.0');
    ...
    if (widget.animation) {
      ...
      _animation = Tween(begin: 0, end: widget.percent).animate(
        CurvedAnimation(parent: _animationController!, curve: widget.curve),
          ...
          setState(() {
            _percent = double.parse(_animation!.value.toString());
          });
    ...
    if (linearGradient != null) {
      if (rotateLinearGradient && progress > 0) {
        ...
        if (_paintLine.strokeCap != StrokeCap.butt) {
          correction = math.atan(_paintLine.strokeWidth / 2 / radius).toInt();
        }
