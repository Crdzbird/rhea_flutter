...
import 'package:rhea_app/utils/crontask/constraint_parser.dart';

export 'package:rhea_app/utils/crontask/constraint_parser.dart'
    show ScheduleParseException;
...
class Schedule {
  ...
  factory Schedule.parse(String cronFormat) {
    final p = cronFormat
        ...
        .toList();
    assert(p.length == 5 || p.length == 6, 'Invalid cron format: $cronFormat');
    final parts = [
      if (p.length == 5) null,
      ...p,
    ];
